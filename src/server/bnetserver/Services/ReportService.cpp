...
namespace Battlenet::Services::V3
{
Report::Report(Session* session) : ReportService(session)
{
}

uint32 Report::HandleSubmitReport(report::v3::client::SubmitReportRequest const* request, NoData* /*response*/,
    std::function<void(ServiceBase*, uint32, ::google::protobuf::Message const*)>& /*continuation*/)
{
    ...
    TC_LOG_INFO("session.rpc", "{} submitted a Battle.net report (issueType {}, source {}, target account {}, club {})",
        _session->GetClientInfo(), issueType, source, targetAccountId, clubId);
